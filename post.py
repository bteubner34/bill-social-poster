...
Runs daily via Render cron job at 3:45 AM MST (10:45 AM UTC)
Posts business + personal content to Instagram and LinkedIn via Zernio
"""
...
START_DATE   = date(2026, 5, 26)   # Day 1 = today
BASE_URL     = "https://zernio.com/api/v1"
HEADERS      = {"Authorization": f"Bearer {ZERNIO_KEY}", "Content-Type": "application/json"}
CDN_BASE     = "https://files.manuscdn.com/user_upload_by_module/session_file/310519663427489423"

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
log = logging.getLogger(__name__)

# ── Pre-uploaded CDN URLs for all 16 days ─────────────────────────────────────
# Format: { day_number: { "business": url, "personal": url } }
# Images were pre-cropped to 4:5 ratio and uploaded to Manus CDN.
# If a CDN url is None the script will upload from local path on the fly.
...
LOCAL_GRAPHICS = {
    1:  {"business": "day1_business_ai_shift_4x5.png",       "personal": "day1_personal_skydiving_4x5.png"},
    2:  {"business": "day2_business_aso.png",                 "personal": "day2_personal_rockclimbing.png"},
    3:  {"business": "day3_business_content.png",             "personal": "day3_personal_scuba.png"},
    4:  {"business": "day4_business_google_ai.png",           "personal": "day4_personal_hawaii.png"},
    5:  {"business": "day5_business_email.png",               "personal": "day5_personal_barn.png"},
    6:  {"business": "day6_business_seo.png",                 "personal": "day6_personal_silfra.png"},
    7:  {"business": "day7_business_seo.png",                 "personal": "day7_personal_whitewater.png"},
    8:  {"business": "day8_business_content.png",             "personal": "day8_personal_horseback.png"},
    9:  {"business": "day9_business_social.png",              "personal": "day9_personal_skiing.png"},
    10: {"business": "day10_business_email.png",              "personal": "day10_personal_partnership.png"},
    11: {"business": "day11_business_hormozi.png",            "personal": "day11_personal_christmas.png"},
    12: {"business": "day12_business_elon.png",               "personal": "day12_personal_iceland_horse.png"},
    13: {"business": "day13_business_data.png",               "personal": "day13_personal_horseback_snow.png"},
    14: {"business": "day14_business_drew.png",               "personal": "day14_personal_breckenridge.png"},
    15: {"business": "day15_business_empathy.png",            "personal": "day15_personal_scuba_wreck.png"},
    16: {"business": "day16_business_silos.png",              "personal": "day16_personal_skiing_michelle.png"},
}
...
CONTENT = {
    1: {
        "business": {
            "text": """For the last three years, brands treated AI like a science experiment. Fund a pilot, issue a press release, repeat. That era is over.
...
#AgenticAI #DigitalMarketing #MarketingStrategy #BusinessGrowth #THATAgency"""
        ...
        "personal": {
            "text": """What stepping out of a perfectly good airplane and running a 22-year agency have in common: you do not survive either by avoiding risk. You survive by understanding it.
...
#Leadership #RiskManagement #Entrepreneurship #Mindset #BusinessStrategy"""
    ...
    2: {
        "business": {
            "text": """The old playbook of "click these ten blue links" is changing fast. Between AI summaries and digital assistants, the way people shop and find information has fundamentally shifted.
...
#SEO #ASO #Ecommerce #SearchMarketing #THATAgency"""
        ...
        "personal": {
            "text": """Rock climbing teaches you something that no boardroom ever will: the only way forward is up, and the only way up is one move at a time.
...
#RockClimbing #Focus #Leadership #Mindset #Entrepreneurship"""
    ...
    3: {
        "business": {
            "text": """AI is the number one priority for marketers right now, but it is also one of the hardest things to implement properly. Why? Because AI only works as well as the data you feed it.
...
#DataStrategy #MarketingTech #ArtificialIntelligence #CustomerExperience #THATAgency"""
        ...
        "personal": {
            "text": """Mountaineering teaches you a lot about pacing. You do not conquer the summit in the first hour. It is a slow, deliberate grind where endurance matters more than speed.
...
#Endurance #BusinessJourney #Mountaineering #Leadership #Mindset"""
    ...
    4: {
        "business": {
            "text": """Recent data shows that Google's AI Overviews are now appearing in 58% of searches — and they are cutting traditional click-through rates by up to 89%.
...
#GoogleAI #SEO #AEO #DigitalMarketing #SearchStrategy #THATAgency"""
        ...
        "personal": {
            "text": """I took this photo in Hawaii recently. "WARNING: Strong Current. High Surf."
...
#Leadership #RiskManagement #BusinessStrategy #Mindset #Adaptability"""
    ...
    5: {
        "business": {
            "text": """Too many leaders treat AI like a vending machine: put a prompt in, get a finished marketing campaign out. That is a recipe for generic, invisible content.
...
#ArtificialIntelligence #MarketingStrategy #Leadership #BusinessGrowth #THATAgency"""
        ...
        "personal": {
            "text": """Building a horse stall from scratch. Tool belt on, sawdust everywhere.
...
#CEO #Leadership #BuilderMindset #ContinuousLearning #Entrepreneurship"""
    ...
    6: {
        "business": {
            "text": """We just wrapped up Google Marketing Live 2026, and the message was clear: AI is now woven into every layer of the advertiser experience.
...
#GoogleMarketingLive #AI #MarketingFundamentals #BusinessStrategy #THATAgency"""
        ...
        "personal": {
            "text": """Diving the Silfra fissure in Iceland. Crystal clear water, visibility that goes on forever, and absolute silence.
...
#ScubaDiving #Focus #Leadership #Clarity #Mindset"""
    ...
    7: {
        "business": {
            "text": """Every time Google updates its algorithm, someone declares SEO is dead. It is not dead. It just evolved.
...
#SEO #AEO #DigitalMarketing #SearchStrategy #THATAgency"""
        ...
        "personal": {
            "text": """Whitewater rafting with the kids. It looks like pure chaos from the outside. Water everywhere, rocks, unpredictable currents.
...
#Leadership #RiskManagement #SystemsThinking #BusinessStrategy #Mindset"""
    ...
    8: {
        "business": {
            "text": """"You do not need more time. You need more focus." — Dan Martell
...
#ContentMarketing #DanMartell #MarketingStrategy #Focus #THATAgency"""
        ...
        "personal": {
            "text": """Riding out in the Colorado plains. When you are on horseback in wide open country, your perspective shifts. You stop looking at the ground right in front of you and start looking at the horizon.
...
#Vision #Leadership #LongGame #Strategy #CEO"""
    ...
    9: {
        "business": {
            "text": """"Attention is the new oil." — Gary Vaynerchuk
...
#SocialMediaMarketing #GaryVee #AttentionEconomy #BrandBuilding #THATAgency"""
        ...
        "personal": {
            "text": """Exploring the waterfalls in New Zealand with Katie.
...
#Curiosity #Exploration #ContinuousLearning #Mindset #Travel"""
    ...
    10: {
        "business": {
            "text": """Gartner just released a report showing that marketing leaders expect AI automation to double by 2028. We are already seeing it happen in email marketing.
...
#EmailMarketing #MarketingAutomation #AI #DataDriven #THATAgency"""
        ...
        "personal": {
            "text": """Michelle and I have been building THAT Agency together for 22 years.
...
#Partnership #BusinessGrowth #AgencyLife #Teamwork #Leadership"""
    ...
    11: {
        "business": {
            "text": """"You do not get paid for the hour. You get paid for the value you bring to the hour." — Alex Hormozi
...
#AlexHormozi #ValueCreation #AI #MarketingAgency #THATAgency"""
        ...
        "personal": {
            "text": """Cutting down our own Christmas tree. No screens, no notifications, just cold air and family tradition.
...
#WorkLifeBalance #MentalHealth #Unplug #FamilyTime #CEO"""
    ...
    12: {
        "business": {
            "text": """"Some people do not like change, but you need to embrace change if the alternative is disaster." — Elon Musk
...
#ElonMusk #SearchMarketing #AEO #Innovation #THATAgency"""
        ...
        "personal": {
            "text": """Iceland. Bright green jacket, orange pants, and an Icelandic horse. You do not always get to choose the conditions, but you always get to choose how you respond.
...
#DataAnalytics #MarketingStrategy #ROI #BusinessGrowth #THATAgency"""
        ...
        "personal": {
            "text": """Horseback riding in the snow with Katie.
...
#Presence #Mindfulness #Family #Leadership #CEO"""
    ...
    14: {
        "business": {
            "text": """"An agency's job is not to say yes to everything. It is to say no to the wrong things." — Drew McLellan
...
#DrewMcLellan #AgencyLife #StrategicPartnership #Marketing #THATAgency"""
        ...
        "personal": {
            "text": """Secret Stash Pizzeria in Breckenridge after a long day in the snow. @secretstashcb
...
#WorkHardPlayHard #Breckenridge #Lifestyle #Entrepreneurship #Balance"""
    ...
    15: {
        "business": {
            "text": """We talk a lot about AI automation, predictive analytics, and Agentic Search. But let's not forget the most important element of marketing: the human on the other side of the screen.
...
#Empathy #HumanConnection #AIMarketing #FutureOfWork #THATAgency"""
        ...
        "personal": {
            "text": """Scuba wreck diving in the dark is not for the faint of heart. It requires absolute focus, precise communication, and a reliance on your training when things get complicated.
...
#Leadership #Preparation #Teamwork #ScubaDiving #BusinessGrowth"""
    ...
    16: {
        "business": {
            "text": """Your SEO team does not talk to your social media team. Your email marketing is completely disconnected from your content strategy. Sound familiar?
...
#IntegratedMarketing #FullFunnel #DigitalStrategy #MarketingROI #THATAgency"""
        ...
        "personal": {
            "text": """Skiing at Crested Butte with Michelle.
...
def upload_image(local_path: str) -> str:
    """Upload a local image to Manus CDN and return the public URL."""
...
def crop_to_4x5(src: str, dst: str) -> None:
    """Crop image to 4:5 aspect ratio (Instagram feed requirement)."""
    ...
    img.save(dst)
    log.info(f"Cropped {src} -> {dst} ({img.size[0]}x{img.size[1]})")
...
def publish_post(text: str, image_url: str, account_ids: list) -> dict:
    ...
    log.info(f"Post created: id={post_id} status={status}")
    return post
...
def main():
    ...
    log.info(f"=== Bill Teubner Social Poster — Day {delta} / {today} ===")
    ...
    accounts = [IG_ACCOUNT, LI_ACCOUNT]
    errors = []

    # Business post
    try:
        log.info(f"--- Day {delta} Business Post ---")
        biz_img = get_image_url(delta, "business")
        biz_text = day_content["business"]["text"]
        publish_post(biz_text, biz_img, accounts)
        log.info("Business post published successfully.")
    except Exception as e:
        log.error(f"Business post failed: {e}")
        errors.append(f"Business: {e}")

    # Personal post
    try:
        log.info(f"--- Day {delta} Personal Post ---")
        personal_img = get_image_url(delta, "personal")
        personal_text = day_content["personal"]["text"]
        publish_post(personal_text, personal_img, accounts)
        log.info("Personal post published successfully.")
    except Exception as e:
        log.error(f"Personal post failed: {e}")
        errors.append(f"Personal: {e}")

    if errors:
        log.error(f"Completed with errors: {errors}")
        sys.exit(1)
    else:
        log.info(f"=== Day {delta} complete. Both posts published. ===")
        sys.exit(0)
